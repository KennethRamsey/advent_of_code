...
void part1()
{
    ...
    // parses single mul(n1,n2)
    var mulParser =
        from _mul in Parse.String("mul(")

        from n1 in Parse.Number
        let num1 = int.Parse(n1)
        where num1 < 1000

        from _comma in Parse.Char(',')

        from n2 in Parse.Number
        let num2 = int.Parse(n2)
        where num2 < 1000

        from _end in Parse.Char(')')

        select num1 * num2;

    // parse the noise, the return type need to match mul's type.
    var anything =
        from x in Parse.AnyChar
        select 0;

    // 1st check if mul can be parsed, 
    // if that fails then parse as anything, 
    // the Many() means keep repeating the full parser of (mul.Or(anything))
    var fullParser = mulParser.Or(anything).Many();

    fullParser.Parse(line)
    .Where(x => x > 0)
    .Sum()
    .Dump();
}
...
void part2()
{
    ...
    var enableMulParsing = true;

    var dontParser =
        from x in Parse.String("don't()")
        let _ = (enableMulParsing = false) // update global state.
        select 0;

    var doParser =
        from x in Parse.String("do()")
        let _ = (enableMulParsing = true) // update global state.
        select 0;

    var mulParser =
        from _mul in Parse.String("mul(")
        where enableMulParsing
    ...
    var anything = Parse.AnyChar.Return(0); // shorter way of doing this.

    // order of parsers matters, put do and don't 1st, then mul, finally anything.
    var fullParser =
            dontParser
            .Or(doParser)
            .Or(mulParser)
