...
void part1()
{
    ...
    .Foreach(p =>
    {
        while (p.Status == Status.none)
        {
            var rule = rules[p.NextRule];

            rule.ApplyAndUpdatePart(p);
            ...
            switch (p.NextRule)
            {
                ...
                case "A":
                    ...
                    break;
                default: break; // apply already set next.
            }
        }
        //p.Dump("done");
    });
...
Part ParseParts(string line, int arg2)
{
...
MyRules ParseRules(string line, int arg2)
{
    var seg = line.Split(new[] { '{', '}' });
    var rules = seg[1].Split(',');
    return new MyRules(seg[0], rules);
}

record Part(int x, int m, int a, int s)
{
    public string NextRule { get; set; } = "in";
...
{
    public string Name;
    public MyRuleStep[] Rules;

    public MyRules(string Name, string[] rules)
    {
        this.Name = Name;
        this.Rules = rules.Select(r => new MyRuleStep(r)).ToArray();
    }

    public void ApplyAndUpdatePart(Part p)
    {
        foreach (var r in Rules)
        {
            var passed = (r.Opt, r.Letter) switch
            {
                (Opt.greater, Letter.x) => p.x > r.GoalNum,
                (Opt.less, Letter.x) => p.x < r.GoalNum,
                (Opt.greater, Letter.m) => p.m > r.GoalNum,
                (Opt.less, Letter.m) => p.m < r.GoalNum,
                (Opt.greater, Letter.a) => p.a > r.GoalNum,
                (Opt.less, Letter.a) => p.a < r.GoalNum,
                (Opt.greater, Letter.s) => p.s > r.GoalNum,
                (Opt.less, Letter.s) => p.s < r.GoalNum,
                (Opt.none, Letter.none) => true // for final rule.
            };

            if (passed)
            {
                p.NextRule = r.NextStep;
                return;
            }
            // else rule failed , try next one.
        }
        // all failed
        p.NextRule = "R";
    }
...
record MyRuleStep
{
    ...
    public bool FAIL { get; set; } // for later.

    public MyRuleStep(string rule)
    {
        if (rule.Contains(':'))
        {
            Letter = rule[0] switch
            {
                'x' => Letter.x,
                'm' => Letter.m,
                'a' => Letter.a,
            ...
            var seg = rule.Split(':');
            GoalNum = int.Parse(seg[0].Substring(2));
            NextStep = seg[1];
        }
    ...
    public bool TestNumber(int num)
    {
        var t = Opt switch
        {
            Opt.greater => num > GoalNum,
            Opt.less => num < GoalNum,
            Opt.none => true,
        };
        return (t && !FAIL) || (!t && FAIL);
    }
}


static Dictionary<string, MyRules> Rules = null;
...
void part2()
{
    ...
    var branches = GetBranches(Rules["in"])
                    .Where(l => l.Last().NextStep == "A");

    branches.Select(fitCounts)
    .Sum()
    .Dump();

    //"167409079868000".Dump("expected");
}

IEnumerable<IEnumerable<MyRuleStep>> GetBranches(MyRules myRules)
{
    for (int i = 0; i < myRules.Rules.Length; i++)
    {
        var list = myRules.Rules
                        .Take(i + 1)
                        .Select((r, idx) => r with { FAIL = idx != i })
                        .ToArray();


        var next = list.Last().NextStep;
        var isNext = next != "R" && next != "A";
        if (isNext)
        {
            var childs = GetBranches(Rules[next]);
            foreach (var c in childs)
                yield return list.Concat(c);
        }
        else
            yield return list;
    }
...
long fitCounts(IEnumerable<MyRuleStep> rulez, int arg2)
{
    var ruleGroups = rulez.GroupBy(x => x.Letter);
    ...
    var letterRanges = Letters.Select(l =>
    {
        var rulesForL = ruleGroups.FirstOrDefault(g => g.Key == l);
        ...
        {
            if (rulesForL.All(fl => fl.TestNumber(i)))
                sum++;
        }
        return sum;
    });

    return letterRanges.Aggregate((x, y) => x * y);
}
