...
void part1()
{
    ...
    .Select(d => d.Split(':'))
    .Select(d => (d[0].Skip(4).CharsToString(), d[1]))
    .Select(d => (d.Item1, d.Item2, d.Item2.Split(';').Select(ballCounts)))
    .Where(d => d.Item3.All(Valid))
    .Select(d => int.Parse(d.Item1))
    .Sum()
    .Dump();
}
...
void part2()
{
    ...
    .Select(d => (d[0].Skip(4).CharsToString(), d[1]))
    .Select(d => (d.Item1, d.Item2, d.Item2.Split(';').Select(ballCounts)))
    .Select(d => Power(d.Item3))
    .Sum()
    .Dump();
}

int Power(IEnumerable<(int, int, int)> ballsArr)
{
    int r = ballsArr.Select(x => x.Item1).Max();
    int g = ballsArr.Select(x => x.Item2).Max();
    int b = ballsArr.Select(x => x.Item3).Max();
    return r * g * b;
}

bool Valid((int, int, int) balls)
{
    return balls.Item1 <= 12
    && balls.Item2 <= 13
    && balls.Item3 <= 14;
}

(int, int, int) ballCounts(string line)
{
    string r, g, b;

    r = Regex.Match(line, @"(\d+) r").Groups[1].Value;
    g = Regex.Match(line, @"(\d+) g").Groups[1].Value;
    b = Regex.Match(line, @"(\d+) b").Groups[1].Value;

    if (r.Length == 0) r = "0";
    if (g.Length == 0) g = "0";
    if (b.Length == 0) b = "0";

    return (int.Parse(r), int.Parse(g), int.Parse(b));
}
