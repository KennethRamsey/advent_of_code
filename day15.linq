...
record operation
{
    public string strCommand, lenslabel, op;
    public int box, focalLen;
    ...
    public operation(string str)
    {
        ...
        this.lenslabel = x[0];
        op = str.Contains("=") ? "=" : "-";
        box = UserQuery.myHash(lenslabel);
        if (op == "=")
        {
            focalLen = int.Parse(x[1]);
        }
    }

    public void doOperation(Dictionary<int, List<operation>> dict)
    {
        if (op == "=")
            replaceOrAdd(dict);
        else
    ...
    public void remove(Dictionary<int, List<operation>> dict)
    {
        if (dict.TryGetValue(box, out var lenses))
        {
            var a = lenses.FirstOrDefault(l => l.lenslabel == lenslabel);
            if (a != null)
                lenses.Remove(a);
        }
    }

    public void replaceOrAdd(Dictionary<int, List<operation>> dict)
    {
        if (dict.TryGetValue(box, out var lenses))
        {
            for (int i = 0; i < lenses.Count; i++)
            {
                if (lenses[i].lenslabel == lenslabel)
                {
                    lenses[i] = this;
                    return;
                }
