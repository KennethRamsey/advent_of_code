...
numAndPositions withSymbols(numAndPositions num, (string line, int lineIndex)[] lines)
{
    ...
    foreach (var digitAndIndexs in num.digits)
    {
        var lin = digitAndIndexs.lineIndex;
        var aboveLin = Math.Max(lin - 1, 0);
        var belowLin = Math.Min(lin + 1, lines.Length - 1);
        ...
        var startIndex = digitAndIndexs.charIndex;
        var prevIndex = Math.Max(startIndex - 1, 0);
        var nextIndex = Math.Min(startIndex + 1, lines[lin].line.Length - 1);
        ...
        var chars = checkLines
                    .Select(l => new[]
                    {
                        new MyChar(l.Item1, startIndex, l.line[startIndex]),
                        new MyChar(l.Item1, prevIndex, l.line[prevIndex]),
                        new MyChar(l.Item1, nextIndex, l.line[nextIndex])
                    })
                    .SelectMany(c => c);

        foreach (var c in chars)
        {
            if (isSymbol(c.Letter))
            {
                syms.Add(c);
            }
        }
